        ...
        <%
            ...
            if (matches != null && !matches.isEmpty()) {
                for (MatchSummaryDTO match : matches) {
                    int placement = match.placement(); // ⚠️ record라면 이거 JSP에서 오류날 수도 있음
                    List<String> imgUrls = match.championImg();
        %>
        <div class="row match-row align-items-center mb-2">
            <div class="col-auto fw-bold">
                [<%= placement %>위] <%= summonerName %>
            </div>
            <div class="col">
                <%
                    if (imgUrls != null) {
                ...
                %>
                <img src="<%= img %>" class="champ-img" alt="champ" style="width:40px; height:40px; margin-right:5px;" />
                <%
                            }
                        }
                    }
